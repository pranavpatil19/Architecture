# .NET Core SDK
FROM mcr.microsoft.com/dotnet/core/sdk:3.1.101-alpine AS dotnetcore-sdk

WORKDIR /source

# Copy Projects
COPY source/Application/Architecture.Application.csproj ./Application/
COPY source/CrossCutting/Architecture.CrossCutting.csproj ./CrossCutting/
COPY source/Database/Architecture.Database.csproj ./Database/
COPY source/Domain/Architecture.Domain.csproj ./Domain/
COPY source/Infra/Architecture.Infra.csproj ./Infra/
...
# Angular
FROM node:13.6-alpine AS angular-build
